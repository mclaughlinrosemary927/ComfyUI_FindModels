...
本文件给后续 Codex/开发者使用。修改本项目时必须优先遵守这里的约束。

## 项目定位

`ComfyUI_FindModels` 是一个 ComfyUI custom node/frontend extension，用来扫描当前工作流，识别缺失模型和缺失节点，提供本地候选、外部模型库迁移、下载来源和缺失节点安装能力。

核心目标不是“猜测模型”，而是尽量复刻 ComfyUI 内置“工作流总览”的依赖识别结果，并且只显示尚未解决的依赖。
...
- `web/find_models.css`：右侧面板 UI。
- `tests/`：回归测试。每次修复真实误报/漏报都要补测试。
- `PROJECT_HANDOFF.md`：项目交接记录，重大行为变化时同步更新。
...
1. **官方目录才算 installed**
   - `diffusion_models` 只有在官方 `models/diffusion_models` 中才算已解决。
   - `text_encoders` 只有在官方 `models/text_encoders` 中才算已解决。
   - 旧目录或兼容目录，例如 `unet`、`clip`，只能作为本地候选，不能让缺失项消失。

2. **同名但路径不一致是 adaptable**
   - 文件名完全一致，大小写可忽略，但保存路径和当前节点值不一致时，应显示“加载本地模型”或候选。
   - 不允许用相似文件名自动替换。
...
5. **点击加载后不能退化**
   - 前端确认过的 `model_value_valid=true` 不能导致本地候选丢失。
   - 如果文件仍不在官方目录，下一次扫描仍应显示 adaptable 候选，而不是变成纯 missing。
...
- Quark 可能因为 token、登录态、限速或权限拒绝直链。不要伪造成功。
- 下载只允许文件名完全一致的候选。
- 下载目标必须是已注册的官方模型分类目录。
- 永远不要覆盖已有模型文件。
...
- 模型已经在本地但不在官方目录时，不能错误隐藏，也不能丢掉“加载本地模型”候选。
